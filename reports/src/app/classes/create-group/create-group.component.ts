        ...
        if(count === expectedStudents || count === (expectedStudents - 1)) {
          ...
          for(let s = 0 ; s < separationTokens.filter(tmp => tmp !== token).length ; s++) {
            ...
            for(let o = 1 ; o < userData.length; o++) {
              ...
              consistent = pastCount === presentCount && pastCount > 0 && presentCount > 0;

              // if not, then this is unlikely to be the deliniator and so move on...
              if(!consistent) {
                console.log(`${subToken} is inconsistent, disregarding...`);
                break;
              } else {
                console.log(`${o} (${subToken}): Pastcount: ${pastCount}, PresentCount: ${presentCount}`);
              }
            }

            if(consistent) {
              // console.log(token, subToken);
              this.generateUserDataRowByRow(token, subToken);
            };
          }
        }
      }
